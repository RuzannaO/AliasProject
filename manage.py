...
class AliasGame:
    ...
    def time_limit_up(self):
        ...
        self.top_level_windows.append(ready_box)
        # print(self.top_level_windows.count())
    ...
    def cleanup_and_exit(self):
        ...
        try:
            ...
            # Destroy all Toplevel windows explicitly before quitting the main window
            for top_window in self.top_level_windows:
                try:
                    if top_window.winfo_exists():
                        print(f"Destroying window: {top_window}")
                        top_window.quit()  # Quit the Toplevel window before destroying
                        top_window.grab_release()  # Release the grab to ensure the window is not modal anymore
                        top_window.withdraw()  # Hide the window before destroying it
                        top_window.destroy()  # Destroy the Toplevel window
                        print("destroyed")
                    else:
                        print(f"Window {top_window} does not exist anymore")
                except Exception as e:
                    print(f"Error destroying Toplevel window {top_window}: {e}")

            # Ensure the root window is hidden first before destroying it
            print("Hiding the root window...")
            self.root.withdraw()  # Hide the main root window

            # Explicitly destroy all widgets in the root window
            for widget in self.root.winfo_children():
                print(f"Destroying widget: {widget}, Type: {type(widget)}")
                widget.destroy()  # Destroy any remaining widgets

            # Schedule root window destruction after idle to ensure all processes are cleaned up
            print("Scheduling root window destruction...")
            self.root.after_idle(self.root.quit)  # Ensures the event loop is exited gracefully
            self.root.after_idle(self.root.destroy)  # Destroys the root window after idle
